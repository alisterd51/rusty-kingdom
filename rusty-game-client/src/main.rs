...
async fn main() -> Result<(), String> {
    ...
    match args.cmd {
        Commands::Bench { size } => {
            let fortress_id = game::fortress::new(&client, &args.url).await?.id;
            for _ in 0..size {
                let _ = game::fortress::gold_collect(&client, &args.url, fortress_id).await;
            }
            let gold = game::fortress::gold_get(&client, &args.url, fortress_id).await?;
            println!("gold: {gold}");
            game::fortress::delete(&client, &args.url, fortress_id).await?;
        }
        ...
        Commands::NewFortress => {
            let fortress = game::fortress::new(&client, &args.url).await?;
            println!("{fortress:?}");
        }
